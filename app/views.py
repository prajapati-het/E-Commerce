...
class ProductView(View):
    def get(self, request):
        ...
        context = {'topwear': topwear,
                   'bottomwear': bottomwear, 'totalitem': totalitem}
...
def Bottomwear(request, data=None):
    if data == None:
        Bottomwear = Product.objects.filter(category='BW')
    elif data == 'Adidas' or data == 'Lewis':
        Bottomwear = Product.objects.filter(category='BW').filter(brand=data)
    elif data == 'above':
        Bottomwear = Product.objects.filter(
            category='BW').filter(discounted_price__gt=6000)
    elif data == 'below':
        Bottomwear = Product.objects.filter(
            category='BW').filter(discounted_price__lt=2000)
...
def Topwear(request, data=None):
    ...
    elif data == 'Lewis' or data == 'Adidas':
        Topwear = Product.objects.all().filter(category='TW').filter(brand=data)
    elif data == 'above':
        Topwear = Product.objects.all().filter(
            category='TW').filter(discounted_price__gte=4000)
    elif data == 'below':
        Topwear = Product.objects.all().filter(
            category='TW').filter(discounted_price__lt=1500)
